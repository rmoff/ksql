...
CREATE table events_per_min WITH (PARTITIONS=1) AS SELECT userid, count(*) AS events FROM clickstream window TUMBLING (size 60 second) GROUP BY userid;
...
CREATE TABLE pages_per_min WITH (PARTITIONS=1) AS SELECT userid, count(*) AS pages FROM clickstream WINDOW HOPPING (size 60 second, advance by 5 second) WHERE request like '%html%' GROUP BY userid ;
...
CREATE TABLE ERRORS_PER_MIN_ALERT WITH (PARTITIONS=1) AS SELECT status, count(*) AS errors FROM clickstream window HOPPING ( size 30 second, advance by 20 second) WHERE status > 400 GROUP BY status HAVING count(*) > 5 AND count(*) is not NULL;

--DROP TABLE IF EXISTS ERRORS_PER_MIN;
CREATE table ERRORS_PER_MIN WITH (PARTITIONS=1) AS SELECT status, count(*) AS errors FROM clickstream window HOPPING ( size 60 second, advance by 5  second) WHERE status > 400 GROUP BY status;
...
--Join using a STREAM
CREATE STREAM ENRICHED_ERROR_CODES WITH (PARTITIONS=1) AS SELECT code, definition FROM clickstream LEFT JOIN clickstream_codes ON clickstream.status = clickstream_codes.code;
-- Aggregate (count&groupBy) using a TABLE-Window
CREATE TABLE ENRICHED_ERROR_CODES_COUNT WITH (PARTITIONS=1) AS SELECT code, definition, COUNT(*) AS count FROM ENRICHED_ERROR_CODES WINDOW TUMBLING (size 30 second) GROUP BY code, definition HAVING COUNT(*) > 1;
...
--CREATE TABLE platinum_errors_per_5_min WITH (PARTITIONS=1) AS SELECT userid, first_name, last_name, city, count(*) AS running_count FROM platinum_errors WINDOW TUMBLING (SIZE 5 MINUTE) WHERE request LIKE '%html%' GROUP BY userid, first_name, last_name, city;
...
CREATE STREAM USER_CLICKSTREAM WITH (PARTITIONS=1) AS SELECT userid, u.username, ip, u.city, request, status, bytes FROM clickstream c LEFT JOIN web_users u ON c.userid = u.user_id;

-- Aggregate (count&groupBy) using a TABLE-Window
--DROP TABLE IF EXISTS USER_IP_ACTIVITY;
CREATE TABLE USER_IP_ACTIVITY WITH (PARTITIONS=1) AS SELECT username, ip, city, COUNT(*) AS count FROM USER_CLICKSTREAM WINDOW TUMBLING (size 60 second) GROUP BY username, ip, city HAVING COUNT(*) > 1;
...
CREATE TABLE CLICK_USER_SESSIONS WITH (PARTITIONS=1) AS SELECT username, count(*) AS events FROM USER_CLICKSTREAM window SESSION (300 second) GROUP BY username;
...
--CREATE TABLE PER_USER_KBYTES WITH (PARTITIONS=1) AS SELECT username, sum(bytes)/1024 AS kbytes FROM USER_CLICKSTREAM window SESSION (300 second) GROUP BY username;

----DROP TABLE IF EXISTS MALICIOUS_USER_SESSIONS;
--CREATE TABLE MALICIOUS_USER_SESSIONS WITH (PARTITIONS=1) AS SELECT username, ip,  sum(bytes)/1024 AS kbytes FROM USER_CLICKSTREAM window SESSION (300 second) GROUP BY username, ip  HAVING sum(bytes)/1024 > 50;
